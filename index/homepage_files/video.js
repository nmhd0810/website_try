...
//1初始；2切换hd；3出错；4全屏切换；5速度切换
	...
	function loadNewVideo(message) {
		...
		if(requsetCount>=3){ //3次仍访问不了，返回错误提示界面
			requsetCount=0;
		...
		switch(urlArr[2]){
			...
			case "video.mukewang.com"://2 video.mukewang-->v1.imooc
				mediaData.mpath[index]=mediaData.mpath[index].replace(/video.mukewang.com/,"v1.imooc.com");
				break;
			case "v1.imooc.com"://2 video-->v1
				mediaData.mpath[index]=mediaData.mpath[index].replace(/v1.imooc.com/,"v1.mukewang.com");
				break;
		}
